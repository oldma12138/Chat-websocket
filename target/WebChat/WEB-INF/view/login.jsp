...
<div class="login-form">
    ...
    <div class="clear"></div>
    <div class="avtar"><img  class="img-circle" id="pic" src="<%=path%>/static/source/img/avtar.png" style="height: 150px;width: 150px;"/></div>
    <form id="login-form" action="<%=path%>/user/login" method="post" onsubmit="return checkLoginForm()">
        <div class="key">
            <input type="text" id="username" name="userid" placeholder="请输入账号" onblur="change()"></span>
        </div>
...
<script>
    function change(){
        ...
        $.post("/user/pic",{ "username":$("#username").val()},function (date) {
            if (date=='false') {
                html="<%=path%>/static/source/pic/404.jpg"
                $("#pic").attr('src',html)
            }
            else{
                html="<%=path%>/static/source/pic/"+date
                $("#pic").attr('src',html)
            }
        })
